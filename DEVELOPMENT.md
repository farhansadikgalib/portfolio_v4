...
- The full collection opens as a numbered index with a sticky search and category rail (with counts), platform and store-status columns, and a screenshot preview that drifts beside the pointer over projects with store imagery. The preview is fine-pointer only and snaps under reduced motion; rows collapse to icon, name, and a summary line on phones.
